...
cutoff and atom types from the checkpoint itself, and runs NVT (Langevin) or NVE.
Writes an extxyz trajectory plus a CSV log of time, temperature and energies.
...
ap.add_argument("--ensemble", choices=["nvt", "nve"], default="nvt")
...
writer.writerow(["step", "time_ps", "T_K", "Epot_eV", "Ekin_eV", "Etot_eV", "ms_per_step"])
...
def snapshot():
    ...
    writer.writerow([step, step * args.dt / 1000, f"{T:.1f}", f"{epot:.4f}", f"{ekin:.4f}", f"{epot + ekin:.4f}", f"{ms:.2f}"])
    log.flush()
    ase.io.write(traj_file, atoms, format="extxyz", append=True)
    print(f"step {step:7d}  t={step * args.dt / 1000:7.3f} ps  T={T:6.1f} K  Epot={epot:11.4f} eV  Etot={epot + ekin:11.4f} eV  {ms:6.2f} ms/step", flush=True)
